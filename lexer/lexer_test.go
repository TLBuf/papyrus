...
`
	...
	for i, tt := range tests {
		tok, issue, ok := lex()
		if issue != nil {
			t.Fatalf("Lex() failed unexpectedly at token %d: %v", i, issue)
		}
		if !ok {
			t.Fatalf("Lex() terminated unexpectedly at token %d", i)
		}
...
`
	...
	b.ReportAllocs()
	for b.Loop() {
		for _, issue := range lexer.Lex(file) {
			if issue != nil {
				b.Fatalf("Lex() failed unexpectedly: %s", issue)
			}
		}
	}
}
